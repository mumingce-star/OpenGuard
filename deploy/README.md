...
风险中心、合规报告共用后端等价情境投影；资源清单按原资源类型分区。共同 AI 指导只在生成前按组调用，浏览器展开、筛选、导出不触发模型。历史报告可用新样式预览，既有下载附件及 SHA 不会静默重写；页面注明数据时间和渲染版本。JSON 保留完整 scan_run，另附 grouping；CSV与资源清单仍保留原始明细语义。

进度百分比仍取原后端状态；动画仅插值至已收到的真实值。ETA 在可测量的 AI 组阶段显示范围，未知阶段显示正在估算，不能当整链承诺。缓存只在当前 API 进程内，重启后首次调用重新生成。部署维护必须先检查活动任务为零，并取得本次负责人确认；不为刷新页面擅自重启正在扫描的服务。
...
维护准备：再次查活动/queued/派发/实际子进程并等待归零；暂停新提交；一致性备份现有数据卷、数据库、上传与报告（已有V4数据时一起备份assessment.db）；保留旧API/Web镜像和原配置。获准后才构建两镜像并启用V4，启动仅新增sidecar，不改scan_runs结构。验收旧报告/新评估/问答/刷新与重启恢复，保留版本清单。失败且无活动任务时恢复旧镜像与原配置；保留新sidecar供排查，旧版不会读取它。运行中有任务时不直接回退或改数据库状态。

本轮备份/历史兼容证据在`output/v4-evidence`；这些隔离验证不关闭Windows、浏览器保存、真人语义审核与P0签收。


### 2026-09-11 本机V4扫描修复补充

5174现代理独立Docker API8011（openguard-v4-api），已开启Git、外部扫描器、durable ZIP和V4/Qwen；原8010进程已空闲关闭，8080生产未更新。数据卷openguard_v4_git_data从备份复制，3条无执行器的queued经用户授权通过registry CAS取消，不删除或重放。保留同样2CPU/4GiB、文件描述符、只读根文件系统、临时磁盘、无新增权限与扫描隔离。旧镜像保留，依赖未升级。本机运行与验收记录见output/v4-git-runtime；它是隔离部署，不是生产发布。

当Git执行器未配置时，POST /api/v1/scans的Git输入现在返回503/git_scanning_unavailable，不创建queued记录。原先仅验证持久化的测试显式注入测试替身；真实uvicorn关闭开关的测试断言拒绝且活动任务为0。
