...
## V4 隔离开发状态（2026-09-11）

本机集成线保留V3扫描补丁，新增可选用途、中文项目评估、同版本报告和本地Qwen3问答。隔离数据副本现由完整工具Docker API 8011与Web 5174启用（替换原无Git执行能力的8010预览），**8080现有生产容器未升级V4**。入口先开首页 `http://127.0.0.1:5174/`；现有容器首页仍是 `http://127.0.0.1:8080/`。启动/预算/接口见[API规格](docs/spec/a3-fastapi-api.md)，最新验收见[进度台账](docs/coordination/PROJECT_PROGRESS.md)。

项目用途是声明，扫描completed不是授权通过；模型解释不提升确定性结论。旧扫描/历史报告保留，新评估存入独立assessment.db。Windows、人工规则/12条复核、浏览器下载和P0签收仍待。下文2026-09-06的耗时和架构状态为历史快照，不代表V4当前性能或最终部署状态。
