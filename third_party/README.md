...
| Hugging Face Hub 公开元数据 API | 第三方公开元数据来源 | 2026-09-20 固定 10 条最小响应快照；逐文件 SHA-256 见 `tests/fixtures/huggingface/resource-profile-v1/manifest.json` | https://huggingface.co/docs/hub/api | Provider 声明待人工核验；本项目不据此断言 API 响应或资源内容的许可证/授权 | 仅采集并再分发必要的公开元数据字段（ID、revision、可见性/gate/disabled 观察值及原始声明）；不下载权重、数据、完整卡片或凭据 | 后续使用、再分发和许可证必须人工核验；`private`、`gated`、`NOASSERTION`、`other` 或成功读取均不代表授权 | OpenGuard 自研快照封装、SHA-256、JSON Pointer Evidence 与 fail-closed 反例；不复制 HF 服务或资源内容 | 已记录固定来源与离线 Java 2/2 回归；`authorization=pending`、正式许可证表达式为空 | 仅提交最小 JSON 元数据 fixture，不提交 API token、模型/数据内容或完整响应 |
| Hamcrest | Java 测试传递依赖 | 3.0；归档 SHA-256 `5d66b6a4a680755cb6ed7cb104fa7835ef644667586ff0737adeb977c39ecdbc` | https://github.com/hamcrest/JavaHamcrest | 归档 `META-INF/LICENSE` 观察为 BSD 3-Clause；正式归一化待人工复核 | 由测试框架传递引入，用于断言匹配 | 分发测试依赖时保留归档中的 copyright、条件与免责声明；未观察到独立 NOTICE | OpenGuard 不修改其源码，仅在测试范围使用 | LICENSE entry SHA-256 已核验；合规状态待核验 | Maven 测试依赖，不提交 jar |
| Mockito Core | Java 测试传递依赖 | 5.23.0；归档 SHA-256 `ae295bebd5d11fab97ab297815dc7617188b86003cbce3dfd5c0d5c3a6cc4a0c` | https://github.com/mockito/mockito | 归档 `LICENSE` 观察为 MIT；正式归一化待人工复核 | 由测试框架传递引入，用于 mocking 支持 | 分发相关副本时保留 copyright 与许可声明；未观察到独立 NOTICE | OpenGuard 不修改其源码，仅在测试范围使用 | LICENSE entry SHA-256 已核验；合规状态待核验 | Maven 测试依赖，不提交 jar |
...
| networknt JSON Schema Validator | Java JSON Schema 校验库 | 2.0.4 | https://github.com/networknt/json-schema-validator/releases/tag/v2.0.4 | Apache-2.0 | 离线执行 Bench 2.0 Draft 2020-12 结构与 format 校验；外部 Schema 自动获取保持关闭 | 分发时保留上游版权与许可证；锁定 Jackson 2 对应发布线 | OpenGuard 自研 Bench Schema、artifact/引用/split/修订/准入语义、稳定诊断和 CLI | 已通过 classpath Schema、正反例和 Maven 测试核验（2026-09-18） | 仅以 Maven 依赖声明引用，不存储库二进制 |
| Exec Maven Plugin | Maven 构建插件 | 3.6.2 | https://www.mojohaus.org/exec-maven-plugin/ | Apache-2.0 | 从 Maven/CI 调用 Bench 2.0 CLI；不参与 manifest 校验语义 | 构建时保留上游许可证，版本固定 | OpenGuard 自研 CLI 参数、JSON 报告、退出码和测试 | 已用正例与策略拒绝反例实跑（2026-09-18） | 仅以 Maven 插件声明引用，不存储插件二进制 |
