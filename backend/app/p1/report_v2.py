...
from .report_v2_integrity import explicit_identity
from .report_v2_sources import validate_fixed_sources, validate_task
...
class ReportV2Service:
    ...
    def _fixed_inputs(
        ...
        try:
            validate_fixed_sources(stored,assessment,scan_id,assessment_id)
        except (ValueError,KeyError,TypeError,AttributeError) as error:
            raise ReportV2ServiceError('upstream_unavailable',reason='report_source_integrity') from error
    ...
    def _task_snapshots(
        ...
        for ref in normalized:
            try:
                task = self.task_store.get_version(scan_id,assessment.id,ref.task_id,ref.version)
            except RemediationStoreError as error:
                raise ReportV2ServiceError('upstream_unavailable',reason='task_store_unavailable') from error
            ...
            try:
                validate_task(task,ref,stored,assessment)
            except (ValueError,KeyError,TypeError,AttributeError) as error:
                raise ReportV2ServiceError('upstream_unavailable',reason='task_source_integrity') from error
    ...
    def create(
        ...
        *,
        ...
        normalized=[]
        for values, model, key in ((task_refs,P1TaskRef,'task_id'),(notice_refs,P1NoticeRef,'draft_id'),(algorithm_refs,P1AlgorithmRef,'kind')):
            if not isinstance(values,list) or not all(isinstance(ref,model) for ref in values):
                raise ReportV2ServiceError('invalid_argument',reason='report_reference_invalid')
            if len({getattr(ref,key) for ref in values})!=len(values):
                raise ReportV2ServiceError('invalid_argument',reason='duplicate_'+{'task_id':'task_ref','draft_id':'notice_ref','kind':'algorithm_ref'}[key])
            normalized.append([ref.model_dump(mode='json') for ref in values])
        identity=explicit_identity(scan_id,assessment_id,*normalized)
        try:
            existing=self.report_store.replay(scan_id,assessment_id,idempotency_key,identity)
        except ReportV2StoreError as error:
            raise ReportV2ServiceError('conflict' if error.code=='idempotency_conflict' else 'upstream_unavailable',reason=error.code) from error
        if existing is not None:
            return P1ReportV2Snapshot.model_validate(existing)
        ...
        if sum(len(v) for v in normalized)>self.max_included_items:
        ...
        run=stored.run
        if sum(len(getattr(run,name)) for name in ('components','ai_assets','evidence','findings','licenses','obligations','remediations')) + len(assessment.resource_evaluations)>self.max_included_items:
            raise ReportV2ServiceError('upstream_unavailable',reason='report_generation_capacity_exceeded')
        ...
        try:
            for chunk in encoder.iterencode(document):
                ...
                if encoded_size>self.max_document_bytes:
                    raise ReportV2ServiceError('upstream_unavailable',reason='report_generation_capacity_exceeded')
