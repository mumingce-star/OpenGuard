...
from .report_v2_integrity import strict_json, validate_document, binding_identity, canonical
...
class ReportV2Store:
    ...
    def _artifact_payloads(
        ...
        *, stored: bool = False,
        ...
        for format_name in ("html", "json"):
            ...
            if len(payload) > (max(32 * 1024 * 1024, self.max_artifact_bytes) if stored else self.max_artifact_bytes):
                raise ReportV2StoreError(
                    "storage_capacity_exceeded"
                )
        ...
        try:
            validate_document(snapshot, result['json'])
        except (ValueError, KeyError, TypeError, RecursionError) as error:
            raise ReportV2StoreError('invalid_argument') from error
        return result
    ...
    def _load(self, db, scan_id, assessment_id, snapshot_id):
        row = db.execute("SELECT snapshot_id,scan_id,assessment_id,created_at,payload,payload_hash FROM report_snapshots WHERE scan_id=? AND assessment_id=? AND snapshot_id=?", (scan_id,assessment_id,snapshot_id)).fetchone()
        if row is None:
            return None
        try:
            rid, sid, aid, created_at, payload, digest = row
            if (rid,sid,aid)!=(snapshot_id,scan_id,assessment_id) or len(payload)>max(16*1024*1024,self.max_snapshot_bytes) or _digest(payload)!=digest:
                raise ValueError('snapshot_integrity')
            snapshot = strict_json(payload)
            self._validate_snapshot(snapshot, scan_id, assessment_id)
            if snapshot['snapshot_id']!=snapshot_id or snapshot['created_at']!=created_at:
                raise ValueError('snapshot_identity')
            rows=db.execute("SELECT format,payload,payload_hash,size_bytes FROM report_artifacts WHERE snapshot_id=?",(snapshot_id,)).fetchall()
            if len(rows)!=2:raise ValueError('artifact_missing')
            artifacts={}
            for fmt, raw, sha, size in rows:
                if fmt in artifacts or type(size) is not int or len(raw)!=size or _digest(raw)!=sha:raise ValueError('artifact_integrity')
                artifacts[fmt]=bytes(raw)
            self._artifact_payloads(snapshot, artifacts, stored=True)
            return snapshot, artifacts
        except (ValueError,KeyError,TypeError,ReportV2StoreError,RecursionError) as error:
            raise ReportV2StoreError('storage_unavailable') from error

    def _existing_request(self, db, scan_id, assessment_id, key, identity):
        row=db.execute("SELECT fingerprint,snapshot_id FROM report_requests WHERE scan_id=? AND assessment_id=? AND request_key=?",(scan_id,assessment_id,key)).fetchone()
        if row is None:return None
        loaded=self._load(db,scan_id,assessment_id,row[1])
        if loaded is None:raise ReportV2StoreError('storage_unavailable')
        snapshot=loaded[0]
        # Legacy fingerprints bind the saved source refs, not a fresh upstream read.
        legacy=_digest(canonical({'schema_version':'1.0',**snapshot['binding']}))
        if row[0]!=legacy:raise ReportV2StoreError('storage_unavailable')
        if binding_identity(snapshot['binding'])!=identity:
            raise ReportV2StoreError('idempotency_conflict')
        return snapshot

    def replay(self, scan_id, assessment_id, key, identity):
        try:
            db=self._connect(readonly=True)
            if db is None:return None
            with closing(db):
                return self._existing_request(db,scan_id,assessment_id,key,identity)
        except (OSError,sqlite3.Error) as error:
            raise self._error(error) from error
        ...
        try:
            with closing(self._connect()) as db, db:
                ...
                if request_identity is not None:
                    existing=self._existing_request(db,scan_id,assessment_id,idempotency_key,request_identity)
                ...
                if request is not None:
                    ...
                    loaded=self._load(db,scan_id,assessment_id,stored_snapshot_id)
                    if loaded is None:raise ReportV2StoreError('storage_unavailable')
                    return loaded[0]
            ...
            # Expected snapshot/key collisions are checked inside BEGIN IMMEDIATE.
            raise ReportV2StoreError("storage_unavailable") from error
    ...
    def get(self, scan_id: str, assessment_id: str, snapshot_id: str) -> dict | None:
        try:
            db=self._connect(readonly=True)
            if db is None:return None
            with closing(db):
                loaded=self._load(db,scan_id,assessment_id,snapshot_id)
                return loaded[0] if loaded is not None else None
        except (OSError,sqlite3.Error) as error:
            raise self._error(error) from error

    def get_artifact(self, scan_id: str, assessment_id: str, snapshot_id: str, format: str) -> bytes | None:
        if format not in {'html','json'}:raise ReportV2StoreError('invalid_argument')
        try:
            db=self._connect(readonly=True)
            if db is None:return None
            with closing(db):
                loaded=self._load(db,scan_id,assessment_id,snapshot_id)
                return loaded[1][format] if loaded is not None else None
        except (OSError,sqlite3.Error) as error:
            raise self._error(error) from error
