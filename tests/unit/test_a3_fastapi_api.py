...
def test_openapi_preserves_p0_routes_with_additive_history_diff_and_graph_get(harness: ApiHarness) -> None:
    ...
    assert {path: sorted(methods) for path, methods in paths.items()} == {
        ...
        "/api/v1/scans/{scan_id}/resource-profiles/refresh": ["post"],
        "/api/v1/scans/{scan_id}/resource-profiles/jobs/{job_id}": ["get"],
        ...
        "/api/v1/scans/{scan_id}/assessments/{assessment_id}/notice-drafts": ["post"],
        "/api/v1/scans/{scan_id}/assessments/{assessment_id}/notice-drafts/{draft_id}": ["get"],
