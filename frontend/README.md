...
进度页直接使用冻结状态接口的0–100 `progress`：首次载入及后续2.5秒轮询到新值时平滑递增，动画不超过后端已经返回的进度；系统启用“减少动态效果”时立即显示实际值。阶段列表继续显示已经完成的步骤。运行中的阶段显示活动提示，AI阶段明确说明Qwen3按风险逐条生成；它不是额外的后端子进度，也不会把长推理伪装成已完成。只有`completed`到100%，`partial`或`failed`停在后端实际终点。
...
当前unit21通过、TypeScript及生产构建通过；既有开发服务和生产preview真实浏览器检查保留。覆盖上传/进度/资源风险证据/四格式报告SHA/刷新不重复POST/手机导航/partial/无效ZIP异步failed/404无mock降级/无不支持接口和浏览器运行错误。运行产物留临时目录，不提交仓库。
