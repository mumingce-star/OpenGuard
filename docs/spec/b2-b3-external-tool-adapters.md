...
- ScanCode 已通过 `ZipIngestionService.ingest_with_tree_consumer` 接入受控 ZIP 物化阶段：仅在 POSIX 环境把只读目录描述符继承给固定子进程，并以子进程自己的 `/proc/self/fd/<n>` 作为输入；扫描前后均校验 inventory seal。工具失败只能产生 partial/error，不得伪造 pass。Syft 与通用 A4 编排仍未接入。
