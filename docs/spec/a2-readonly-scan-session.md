...
## 2026-09-07：只读剩余额度与生产读取配置

可信parser可以查询`remaining_read_bytes`，它只返回原会话已扣减后的剩余字节，不暴露目录、不补充额度；会话结束、线程越界或已有failure仍拒绝。扫描单文件默认额度从2MiB改为4MiB，总读取硬上限仍为16MiB；请求不得提高服务端额度。Pipeline使用4MiB/16MiB，AI资源遍历受4MiB/文件、16MiB/累计、4096文件及真实剩余额度共同限制。跳过或解码失败仍partial，封印/Hash/文件边界与额度预扣不变。

同轮增加可信parser的`read_many_bytes(tuple_of_paths)`：最多4096条路径，预扣整个批次的真实字节；批次前后各一次完整封印校验，逐文件仍经原fd/Hash读取器。只有批次全部通过才返回字节字典，任何文件或整树变化均使整批失败、零结果返回。`read_bytes`复用同一路径处理单文件；没有开放文件路径/描述符/写入能力。AI识别改用该批次入口，避免每读一文件都重复哈希全仓库。真实临时文件系统测试覆盖已读文件被后续篡改时整批不返回、额度在打开前预扣、重复路径拒绝、会话过期和140文件只做两次整树校验。
