...
更新时间：2026-09-02 20:40（Asia/Shanghai）
...
| B1-2 Python P0映射与CLI纵切 | P0 | Sol→Terra→Luna→Sol→Root | 已完成 | 冻结DTO映射为P0 `Component/Evidence`；UUIDv5稳定ID、证据定位/哈希/时间、exact pin、direct/VCS、partial诊断、固定时钟与旧CLI兼容；Sol终审发现并关闭2项P1；Terra 45项、Luna独立32项、全量355项、P0 46项通过；`EVD-B1-PYTHON-P0-CLI-001` 已绑定 `daee8a8` | B1总包下一步进入 JS/TS manifest 与选定 lockfile；本纵切不代表许可证、依赖求解或完整报告 | 实现提交已冻结；待推送 `feat/b1-p0-mapper-cli` |
...
| B1 | Python/JS依赖解析 | Terra | 进行中 | B1-1 Python requirements/pyproject parser及B1-2 P0映射/CLI纵切已完成；当前Terra B1-2 45项、Luna独立32项、全量355项通过 | 实现 `package.json`/`package-lock.json` 与选定 Python/JS lockfile；再进入多来源合并 | 9月4日-11日 |
...
| 2026-09-02 | B1-2 Python P0映射与CLI纵切 | `feat/b1-p0-mapper-cli` | `daee8a8`（不可变实现提交） | Python P0 mapper、新依赖CLI、45项实现测试、32项独立测试、规格/终审、运行说明、AI/协作与进度记录 | 证据已绑定；待推送 |
