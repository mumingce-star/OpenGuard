...
### 2026-09-08 持久容量准入（不等于物理硬配额）

按用户确认保留历史报告，新增Git/ZIP共享POST准入：2GiB水位、每queued/running及新任务256MiB余量、文件系统512MiB底线；包含DB/WAL/SHM/上传/派发/报告及未知文件，只排除独立挂载workspace，普通本地workspace计入。异常目录/符号链接/不可核实容量失败关闭，不删除任何已有结果。入口503明示原因，GET不受限。单API进程受理锁覆盖上传至记录创建，202之后靠持久active计数保留余量。

边界：这是准入保护，不是卷硬限；已受理任务、外部写入及无界快照仍可能突破水位。持久物理硬配额未关闭，多进程一致性不声称支持。G2最终签收须明确此限制；保留后续验收，不用文案覆盖旧未通过项。临时对象仅由既有生命周期清理；不新增旧报告自动删除。
